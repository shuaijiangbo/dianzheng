<template>
  <!--  <h1>分页组件</h1>-->
  <div class="pages">
    <button class="firstpage" :disabled="sellPage.pageNum==0" @click="sellPage.pageNum=0">首页</button>
    <button class="prevpage" :disabled="sellPage.pageNum==0" @click="sellPage.pageNum--" >上一页</button>
    ...
    <button @click="sellPage.pageNum++" class="nextpage" :disabled="sellPage.pageNum==pageObj.totalPage-1">下一页</button>
    <button class="lastpage" @click="sellPage.pageNum=pageObj.totalPage-1" :disabled="sellPage.pageNum==pageObj.totalPage">末页</button>
...
/*  import url from '@/api/serviceAPI.config.js';*/
  export default {
    ...
    methods:{
      searchList(prames){
        get(this.sellUrl,this.sellPage).then((res) => {
            ...
            this.pageObj.pageNum=res.data.pageNum+1;
    ...
    chooseIndex(pages){
        this.sellPage.pageNum=pages-1;
     },
    parmsExtend(){
        console.log(this.sendParms.url);
        this.sellUrl=this.sendParms.url;
        delete this.sendParms.url;//删除属性
        this.sellPage=Object.assign(this.sellPage,this.sendParms);
        this.searchList();
      }
    },
    created(){
