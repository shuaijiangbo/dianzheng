        ...
        <template>
          <div class="table-box">
            ...
            <div class="table-content">
              <el-form size="mini" :model="firstItem.tableData"  :rules="rules">
                <el-table
                  ...
                  <el-table-column
                    ...
                    <template scope="scope">
                      <el-form-item style="margin-bottom: 0"
                                    :prop="scope.$index+'.personNameInfo'"
                                    :rules="rules.personNameInfo">
                  ...
                  <el-table-column
                    ...
                    <template scope="scope">
                      <el-form-item style="margin-bottom: 0"
                                    :prop="scope.$index+'.genderInfo'"
                                    :rules="rules.genderInfo">
                  ...
                  <el-table-column
                    ...
                    <template scope="scope">
                      <el-form-item style="margin-bottom: 0"
                                    :prop="scope.$index+'.credentialsTypeInfo'"
                                    :rules="rules.credentialsTypeInfo">
...
<script>
  ...
  export default {
    ...
    data(){
      return {
        rules: {
          personIdcodeInfo: [{ required: true, message: ' ', trigger: 'blur' }],
          genderInfo: [{ required: true, message: ' ', trigger: 'blur' }],
          credentialsTypeInfo: [{ required: true, message: ' ', trigger: 'blur' }],
          personNameInfo: [{ required: true, message: ' ', trigger: 'blur' }],
        },
        orgId:'',
        roleType:'',
        timer:null,
      ...
      savePlayer(){
        ...
        this.timer=setTimeout(()=>{
          if(_self.roleType=='4'||!_self.allowApply){
            ...
            /*this.$router.push({path:'/index/classStatus',query:{}})*/
            /*     this.$nextTick(function(){
                   var jjj = this.$refs.jjj;
                   let point=true;
                   for(var i in jjj){
                     let single= jjj[i];
                     single.validate((valid) => {
                       if(valid){
                         point=true;
                       }else{
                         this.$notify({
        ...
        post(url.searchPlayer,{personName  :queryString,testType :testType,testLevel :testLevel,type  :this.matchId})
          .then(res=>{
            ...
            for(let i of this.libraryData){
              ...
              let personIdcodeInfo=i.personIdcodeInfo?'('+i.personIdcodeInfo+')':'(空)';

              i.value = personNameInfo+personIdcodeInfo;  //将想要展示的数据作为value
            }
            cb(this.libraryData);
          })
      },
      handleSelect(item) {
        /* item.perAllow=item.personIdcodeInfo?true:false;
         item.TypeInfo=item.credentialsTypeInfo?true:false;
         item.genderInfollow=item.genderInfo?true:false;*/
        console.log(item);
        if(item.credentialsTypeInfo=='0'){
          item.credentialsTypeInfo='';
        }
        Object.assign(item,this.firstClass[this.classIndex].form,{orgId:this.orgId,matchId:1});
        ...
        this.firstClass[this.classIndex].tableData[this.typeIndex]=item;
      },
